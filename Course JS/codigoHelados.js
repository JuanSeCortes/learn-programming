dineroCofla = prompt("¿Cuanto dinero tienes Cofla?")
dineroRoberto = prompt("¿Cuanto dinero tiene Roberto?")
dineroPedro = prompt("¿Cuanto dinero tienes Pedro?")

// Este es el caso de Cofla 

if (dineroCofla >= 0.6 && dineroCofla <1) {
    alert("Cofla, comprate el helado de agua");
    alert("Y te sobran" + (dineroCofla - 0.6));
}

else if (dineroCofla >= 1 && dineroCofla < 1.6) {
    alert("Cofla, comprate el helado de Vainilla");
    alert("Y te sobran " + (dineroCofla - 1));
}
else if (dineroCofla >= 1.6 && dineroCofla < 1.7) {
    alert("Cofla, comprate el helado de Limon");
    alert("Y te sobran " + (dineroCofla - 1.6));
}
else if (dineroCofla >= 1.7 && dineroCofla < 1.8) {
    alert("Cofla, comprate el helado de Oreo");
    alert("Y te sobran " + (dineroCofla - 1.7));
}
else if (dineroCofla >= 1.8 && dineroCofla < 2.9) {
    alert("Cofla, comprate el helado de Macadamia");
    alert("Y te sobran " + (dineroCofla - 1.8));
}
else if (dineroCofla >= 2.9) {
    alert("Cofla, comprate el helado con confites o el pote de 1/4kg");
    alert("Y te sobran " + (dineroCofla - 2.9));
}
else {
    alert("Cofla, estas re pobre manitooo!")
}
 
// Este es el caso de Roberto

if (dineroRoberto >= 0.6 && dineroRoberto <1) {
    alert("Roberto, comprate el helado de agua");
    alert("Y te sobran" + (dineroRoberto - 0.6));
}

else if (dineroRoberto >= 1 && dineroRoberto < 1.6) {
    alert("Roberto, comprate el helado de Vainilla");
    alert("Y te sobran " + (dineroRoberto - 1));
}
else if (dineroRoberto >= 1.6 && dineroRoberto < 1.7) {
    alert("Roberto, comprate el helado de Limon");
    alert("Y te sobran " + (dineroRoberto - 1.6));
}
else if (dineroRoberto >= 1.7 && dineroRoberto < 1.8) {
    alert("Roberto, comprate el helado de Oreo");
    alert("Y te sobran " + (dineroRoberto - 1.7));
}
else if (dineroRoberto >= 1.8 && dineroRoberto < 2.9) {
    alert("Roberto, comprate el helado de Macadamia");
    alert("Y te sobran " + (dineroRoberto - 1.8));
}
else if (dineroRoberto >= 2.9) {
    alert("Roberto, comprate el helado con confites o el pote de 1/4kg");
    alert("Y te sobran " + (dineroRoberto - 2.9));
}
else {
    alert("Roberto, estas re pobre manitooo!")
}

// Este es el caso para Pedro

if (dineroPedro >= 0.6 && dineroPedro <1) {
    alert("Pedro, comprate el helado de agua");
    alert("Y te sobran" + (dineroPedro - 0.6));
}

else if (dineroPedro >= 1 && dineroPedro < 1.6) {
    alert("Pedro, comprate el helado de Vainilla");
    alert("Y te sobran " + (dineroPedro - 1));
}
else if (dineroPedro >= 1.6 && dineroPedro < 1.7) {
    alert("Pedro, comprate el helado de Limon");
    alert("Y te sobran " + (dineroPedro - 1.6));
}
else if (dineroPedro >= 1.7 && dineroPedro < 1.8) {
    alert("Pedro, comprate el helado de Oreo");
    alert("Y te sobran " + (dineroPedro - 1.7));
}
else if (dineroPedro >= 1.8 && dineroPedro < 2.9) {
    alert("Pedro, comprate el helado de Macadamia");
    alert("Y te sobran " + (dineroPedro - 1.8));
}
else if (dineroPedro >= 2.9) {
    alert("Pedro, comprate el helado con confites o el pote de 1/4kg");
    alert("Y te sobran " + (dineroPedro - 2.9));
}
else {
    alert("Pedro, estas re pobre manitooo!")
}

